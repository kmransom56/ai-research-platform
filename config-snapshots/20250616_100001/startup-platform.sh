...
# CRITICAL: Validate configuration before startup to prevent drift
echo ""
echo "🔍 CONFIGURATION VALIDATION"
echo "=========================="
if [[ -f "$PLATFORM_DIR/config-validation.sh" ]]; then
    bash "$PLATFORM_DIR/config-validation.sh"
    if [[ $? -ne 0 ]]; then
        echo "❌ Configuration validation failed - stopping startup"
        exit 1
    fi
else
...
# Check AI Research Services
echo ""
echo "🤖 Checking AI Research Services"
echo "==============================="

if curl -s --max-time 5 "http://100.123.10.72:11020" &> /dev/null; then
    echo "✅ Perplexica Search AI running (Port 11020)"
else
    echo "⚠️ Perplexica Search AI not responding (Port 11020) - External Docker Service"
fi

if curl -s --max-time 5 "http://100.123.10.72:11021" &> /dev/null; then
    echo "✅ SearXNG Search Engine running (Port 11021)"
else
    echo "⚠️ SearXNG Search Engine not responding (Port 11021) - External Docker Service"
fi

if curl -s --max-time 5 "http://100.123.10.72:8080/api/config" &> /dev/null; then
    echo "✅ OpenWebUI running (Port 8080)"
else
    echo "⚠️ OpenWebUI not responding (Port 8080)"
fi
...
declare -A services=(
    ...
    ["HTTP Gateway"]="http://100.123.10.72:11081"
...
echo "   🔗 HTTP Gateway: http://100.123.10.72:11081 ✅"
echo "   🔒 HTTPS Gateway: http://100.123.10.72:11082 ✅"
echo ""
...
echo "   🔍 Perplexica Search: http://100.123.10.72:11020/perplexica"
echo "   🔍 SearXNG Search: http://100.123.10.72:11021"
echo "   💬 OpenWebUI: http://100.123.10.72:8080"
echo ""
echo "🦙 AI BACKEND:"
...
cat > "$PLATFORM_DIR/platform-status.json" << EOF
{
    ...
    "services": {
        ...
        "ai_research": {
            "perplexica": {
                "port": 3999,
                "status": "$(curl -s http://100.123.10.72:3999 &> /dev/null && echo 'running' || echo 'stopped')",
                "url": "http://100.123.10.72:3999"
            },
            "searxng": {
                "port": 4000,
                "status": "$(curl -s http://100.123.10.72:4000 &> /dev/null && echo 'running' || echo 'stopped')",
                "url": "http://100.123.10.72:4000"
            },
            "openwebui": {
                "port": 8080,
                "status": "$(curl -s http://100.123.10.72:8080/api/config &> /dev/null && echo 'running' || echo 'stopped')",
                "url": "http://100.123.10.72:8080"
            }
        }
    }
}
