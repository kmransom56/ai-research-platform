...
if curl -s --max-time 5 "http://100.123.10.72:3999" &> /dev/null; then
    echo "✅ Perplexica Search AI running (Port 3999)"
else
    echo "⚠️ Perplexica Search AI not responding (Port 3999)"
fi

if curl -s --max-time 5 "http://100.123.10.72:4000" &> /dev/null; then
    echo "✅ SearXNG Search Engine running (Port 4000)"
else
    echo "⚠️ SearXNG Search Engine not responding (Port 4000)"
fi
...
echo "   🔍 SearXNG Search: http://100.123.10.72:4000"
