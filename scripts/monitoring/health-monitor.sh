...
monitor_services() {
    ...
    local services=(
        ...
        "Webhook Server|http://100.123.10.72:11002/health|cd $SCRIPT_DIR && nohup node runtime-data/webhook-server.js > $SCRIPT_DIR/logs/webhook-server.log 2>&1 & echo \$! > $SCRIPT_DIR/pids/webhook-server.pid|$SCRIPT_DIR/pids/webhook-server.pid"
        "Magentic-One Server|http://100.123.10.72:11003/health|cd $SCRIPT_DIR && source .venv/bin/activate && nohup python python/services/magentic_one_server.py > $SCRIPT_DIR/logs/magentic-one.log 2>&1 & echo \$! > $SCRIPT_DIR/pids/magentic-one.pid|$SCRIPT_DIR/pids/magentic-one.pid"
