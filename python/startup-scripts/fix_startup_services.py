...
class StartupServicesFixer:
    ...
    def stop_all_services(self):
        ...
        for service in services:
            try:
                subprocess.run(
                    f"systemctl --user stop {service}", shell=True, check=False
                )
                subprocess.run(
                    f"systemctl --user disable {service}", shell=True, check=False
                )
...
[Service]
Type=simple
ExecStart=/usr/bin/node {self.platform_dir}/runtime-data/webhook-server.js
WorkingDirectory={self.platform_dir}
Environment=NODE_ENV=production
Environment=WEBHOOK_PORT=11025
...
start_service "Webhook Server" \\
    "node {self.platform_dir}/runtime-data/webhook-server.js" \\
...
wait_for_service "Webhook Server" "http://100.123.10.72:11025/health" 10
...
echo "   🔗 Webhook Server: http://100.123.10.72:11025/health"
...
{{
    ...
    "services": {{
        ...
        "webhook_server": "$(curl -s http://100.123.10.72:11025/health &> /dev/null && echo 'running' || echo 'stopped')",
...
"""
    ...
    def create_cron_backup(self):
        ...
        try:
            ...
            if (
                "@reboot" not in existing_cron
                or "startup-platform.sh" not in existing_cron
            ):
    ...
    def enable_services(self):
        ...
        try:
            ...
            subprocess.run(
                "systemctl --user enable ai-research-platform-startup.service",
                shell=True,
                check=True,
            )
            subprocess.run(
                "systemctl --user enable ai-research-platform-startup.timer",
                shell=True,
                check=True,
            )
            ...
            # Start timer
            subprocess.run(
                "systemctl --user start ai-research-platform-startup.timer",
                shell=True,
                check=True,
            )
...
check_service "Webhook Server" "http://100.123.10.72:11025/health"
...
def main():
    ...
    print("\n📋 Systemd Commands:")
    print(
        "   📊 Check timer: systemctl --user status ai-research-platform-startup.timer"
    )
